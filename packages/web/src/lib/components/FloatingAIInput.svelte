...
import { Close, Send, Sparkles, X } from "./icons";
...
<button
	...
	class={cn(
		'fixed bottom-6 right-6 z-[101] w-14 h-14 flex items-center justify-center rounded-full shadow-2xl transition-all duration-300 active:scale-90',
		isOpen
			? 'bg-surface-base border border-subtle-base text-foreground-secondary hover:bg-surface-raised'
...
{#if isOpen}
	<div class="fixed bottom-24 left-4 right-4 z-[100]" data-tour="ai-input">
		<div class="max-w-4xl mx-auto">
			...
			<!-- AI Input Box -->
			<div class={cn(
				'relative bg-surface-base border rounded-2xl shadow-2xl transition-all duration-300 overflow-hidden animate-in slide-in-from-bottom-2 duration-300',
				selectedTasks.length > 0 ? 'border-accent shadow-accent/10' : 'border-subtle-base hover:border-primary/20'
			)}>
				<!-- Close Button -->
				<button
					onclick={() => isOpen = false}
					class="absolute top-3 right-3 z-10 p-1.5 rounded-full text-foreground-secondary hover:bg-surface-raised hover:text-foreground-base transition-colors"
				...
				<div class="flex items-end gap-3 p-4 pr-12">
					<div class="flex-1">
						<textarea
							bind:value={input}
