...
/**
 * Apply error recovery strategy with progressive escalation
 */
export async function applyErrorRecovery(
	errorType: string,
...
export function createErrorActions(
	...
	switch (errorType) {
		case "oauth":
			...
			actions.push({
				...
				action: () => {
					...
					errorStateManager.clearErrorsByType("oauth");
				},
			});
			break;
		...
		case "sync":
			actions.push({
				...
				action: async () => {
					if (context?.integrationId) {
						try {
							await fetch(`/api/integrations/${context.integrationId}/sync`, {
								method: "POST",
							});
						} catch (error) {
							handleApiError(error, { action: "retry_sync" });
						}
					}
				},
				primary: true,
			...
			actions.push({
				id: "disconnect_integration",
				label: "Disconnect",
				action: async () => {
					if (context?.integrationId) {
						try {
							await fetch(`/api/integrations/${context.integrationId}`, {
								method: "DELETE",
							});
						} catch (error) {
							handleApiError(error, { action: "disconnect_integration" });
						}
					}
				},
				destructive: true,
		...
		case "network":
			actions.push({
				...
				label: "Retry",
				action: async () => {
					// Generic retry - context should contain retry function
					if (
						context?.retryFunction &&
						typeof context.retryFunction === "function"
					) {
						await (context.retryFunction as () => Promise<void>)();
					}
				},
				primary: true,
			});
	...
	actions.push({
		...
		action: () => {
			if (context?.errorId) {
				errorStateManager.removeError(context.errorId as string);
			}
		},
	});
