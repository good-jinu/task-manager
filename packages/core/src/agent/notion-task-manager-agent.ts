...
import { createNotionExecutorFactory, type ExecutionStep } from "./tools";
...
/**
 * NotionTaskManagerAgent orchestrates Notion task management using AI SDK tool calling
 */
export class NotionTaskManagerAgent {
	...
	async execute(params: AgentExecuteParams): Promise<AgentExecutionResult> {
		...
		const factory = createNotionExecutorFactory(
			notionManager,
			databaseId,
